...
class PairAllClassGen(tf.keras.utils.Sequence):
    ...
    def __get_input(self, folder):
        ...
        full_images = [np.load(x) for x in images[:self.sequence_length]]
        cutout_images = [np.load(x) for x in images[self.sequence_length:]]
        full_images = [abs(full_images[x]-full_images[x-1]) for x in range(1, self.sequence_length)]
        cutout_images = [abs(cutout_images[x]-cutout_images[x-1]) for x in range(1, self.sequence_length)]
