...
AUG_ALL_CLASS_PRIOR_TRAIN_DATA_DIR = (
    f"./data/{FLARE_CLASS}_lstm_data_augmented_prior/train/"
)
AUG_ALL_CLASS_PRIOR_VAL_DATA_DIR = (
    f"./data/{FLARE_CLASS}_lstm_data_augmented_prior/val/"
)
...
mc = ModelCheckpoint(
    f"{BEST_TRAINED_MODELS_DIR}/{output_name}.h5",
    monitor="val_loss",
    save_best_only=True,
)
...
adam_fine = Adam(
    learning_rate=0.001, beta_1=0.9, beta_2=0.999, decay=0.0002, amsgrad=False
)
lr_schedule = tf.keras.optimizers.schedules.ExponentialDecay(
    initial_learning_rate=1e-3, decay_steps=10000, decay_rate=0.9
)
optimizer = tf.keras.optimizers.SGD(learning_rate=lr_schedule)
model.compile(loss="categorical_crossentropy", optimizer=adam_fine, metrics=metrics)
...
epochs = 10
history = model.fit(
    traingen, validation_data=valgen, epochs=epochs, callbacks=callbacks_list
)
